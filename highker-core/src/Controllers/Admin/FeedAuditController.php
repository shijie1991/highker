...
class FeedAuditController extends AdminController
{
    protected $title = '动态审核';
    ...
    protected function grid(): Grid
    {
        ...
        $feed = Feed::query()->withoutGlobalScope('approve')
            ->where('status', FeedStatus::PENDING)
            ->with(['content', 'user', 'topics'])
            ->orderByDesc('id')
        ;
        ...
        return Grid::make($feed, function (Grid $grid) {
            ...
            // $grid->column('topic.name', '所属圈子')->display(function ($name) {
            //     return $name ? $name : '未选择';
            // });
            ...
            $grid->column('content.text', '内容')->setAttributes(['class' => 'content'])
                ...
                ->display(function ($content) {
                    ...
                    $tooltip->title(str_replace(PHP_EOL, '', $content))
                        ->maxWidth(300)
                        ->blue()
                        ->bottom()
                    ;
            ...
            $grid->filter(function (Grid\Filter $filter) {
                ...
                $filter->in('topic_id', '所属圈子')->multipleSelect($this->getTopicList());
            });
        });
    }
